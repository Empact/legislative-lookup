class AddStateLegislativeDistricts < ActiveRecord::Migration
  def self.up
    ...
    (1..72).each do | n |
      ...
      `PGPASSWORD="#{password}" psql -d #{db} -h #{host} -U #{user} -f #{RAILS_ROOT}/db/state/lower/senate_lower_#{n}.sql`
      `PGPASSWORD="#{password}" psql -d #{db} -h #{host} -U #{user} -f #{RAILS_ROOT}/db/state/upper/senate_upper_#{n}.sql`
    end
